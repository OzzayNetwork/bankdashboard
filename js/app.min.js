	...
	if ($("#data-table")[0]) {
		$("#data-table").DataTable({
			...
			initComplete: function (a, b) {
				$(this).closest(".dataTables_wrapper").find(".dataTables__top").prepend('<div class="dataTables_buttons hidden-sm-down actions"><span class="actions__item zmdi zmdi-print" data-table-action="print" title="print the records" /><span class="actions__item zmdi zmdi-fullscreen" data-table-action="fullscreen" /><div class="dropdown actions__item" title="download the records"><i data-toggle="dropdown" class="zmdi zmdi-download" /><ul class="dropdown-menu dropdown-menu-right"><a href="" class="dropdown-item" data-table-action="excel">Excel (.xlsx)</a><a href="" class="dropdown-item" data-table-action="csv">CSV (.csv)</a></ul></div></div>')
			}
    ...
    if ($("#paybill-data")[0]) {
        ...
        "order": [[ groupColumn, 'asc' ]],
            ...
            api.column(groupColumn, {page:'current'} ).data().each( function ( group, i ) {
                if ( last !== group ) {
                    $(rows).eq( i ).before(
                        '<tr class="group data-group"><td colspan="62">'+group+'</td></tr>'
                    );
...
$("#dropzone-upload")[0] && (Dropzone.autoDiscover = !1), $(document).ready(function () {
	var groupColumn =2;
    if ($("#banknaration-data")[0]) {
		
		
		
        $("#banknaration-data").DataTable({
...
$("#dropzone-upload")[0] && (Dropzone.autoDiscover = !1), $(document).ready(function () {
	var groupColumn =1;
    if ($(".recon-data-table")[0]) {
		
		
		
        $(".recon-data-table").DataTable({
...
$(document).ready(function(){
	...
	function checkoffset(){
		...
		if(scrollbarlocation>=topoffset){
			$('.scroller').addClass('rotate180');
			console.log("it is big");
			var link='#top';
			var title="M-Pesa transfers to grandfather pay bill";
			$('.scroller').attr("href",link);
			$('.scroller').attr("title",title);

		}
		if(scrollbarlocation<topoffset){
			$('.scroller').removeClass('rotate180');
			console.log("it is no longer big");
			var title="grandfather pay bill transfers to CBK account";
			var link='#bottom';
			$('.scroller').attr("href",link);
			$('.scroller').attr("title",title);
	...
	// upload banks statement
	$('body').on('click','#upload-bank', function () {
		$('.drop-bank-container').addClass('fadeIn');

		$('.drop-bank-container').removeClass('d-none');

		$('.drop-bank-container').removeClass('fadeOut');
		$('.drop-bank-container').removeClass('d-none');
		$('.drop-bank-container .card').addClass('bounceUp');
		
	});
	// upload bank stateent form

	// upload mpesa statement
	$('body').on('click','#upload-mpesa', function () {
		$('.drop-mpesa-container').addClass('fadeIn');

		$('.drop-mpesa-container').removeClass('d-none');

		$('.drop-mpesa-container').removeClass('fadeOut');
		$('.drop-mpesa-container').removeClass('d-none');
		$('.drop-mpesa-container .card').addClass('bounceUp');
		
	});
	// upload mpesa stateent form
	...
	//  show mpesa in reconmet
	 $('body').on('click','.mpesa-trans-link', function(){
		 var thetable=$('.mpesa-transactions');
		 thetable.removeClass("zoomOutLeft").removeClass('d-none').addClass('zoomInRight').siblings().addClass('d-none').removeClass('zoomInRight').addClass('zoomOutLeft'); 
	 });

	 //  show bank transactions in reconmet
	 $('body').on('click','.bank-trans-link', function(){
		var thetable=$('.bank-transactions');
		thetable.removeClass("zoomOutLeft").removeClass('d-none').addClass('zoomInRight').siblings().addClass('d-none').removeClass('zoomInRight').addClass('zoomOutLeft'); 
	});

	//  show CRFk transactions in reconmet
	$('body').on('click','.crf-trans-link', function(){
		var thetable=$('.CRF-transactions');
		thetable.removeClass("zoomOutLeft").removeClass('d-none').addClass('zoomInRight').siblings().addClass('d-none').removeClass('zoomInRight').addClass('zoomOutLeft'); 
	});

	//  show CRFk transactions in reconmet
	$('body').on('click','.debit-trans-link', function(){
		var thetable=$('.debit-transactions');
		thetable.removeClass("zoomOutLeft").removeClass('d-none').addClass('zoomInRight').siblings().addClass('d-none').removeClass('zoomInRight').addClass('zoomOutLeft'); 
	});
	// nav link clicking 

	// getting table text
	$('.bank-transactions td').on('click','button', function(){
		var tittle=$(this).parent().siblings().eq(0).html();
		var amount=$(this).parent().siblings().eq(2).html();
		var date=$(this).parent().siblings().eq(1).html();
		console.log(tittle);
		$('.b_title').text(tittle);
		$('.b_date').text(date);
		$('.b_amount').text("KES "+amount);
	});

	$('.mpesa-transactions td').on('click','button', function(){
		var tittle=$(this).parent().siblings().eq(0).children('h6').html();
		var amount=$(this).parent().siblings().eq(3).html();
		var date=$(this).parent().siblings().eq(1).html();
		var details=$(this).parent().siblings().eq(0).children('p').html();

		console.log(tittle);
		$('.m_title').text(tittle);
		$('.m_date').text(date);
		$('.m_amount').text("KES "+amount);
		$('.m_details').text(details);
	});

	$('.CRF-transactions td').on('click','button', function(){
		var tittle=$(this).parent().siblings().eq(0).html();
		$('.d_title').text(tittle);
		$('.delete-item-notification').addClass('fadeIn');
		$('.delete-item-notification').removeClass('d-none');
	...
	$('table td').on('click','.delete-item', function(){
		$('.delete-item-notification').addClass('fadeIn');
		$('.delete-item-notification').removeClass('d-none');
		var title=$(this).parent().parent().parent().parent().siblings().eq(0).html();
		$('.d_title').text(title);
	});

	$('.delete-item-notification').on('click','.close-delete', function(){
		$('.delete-item-notification').addClass('d-none');
		$('.delete-item-notification').removeClass('fadeIn');
